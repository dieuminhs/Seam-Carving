...
__global__ void calculateImportancyKernel(int * inPixels, int width, int height,
				int * nextPixels)
{
	// TODO
	// blockSize = Width, gridSize = 1
	...
	{	
		// if(threadIdx.x == 0)
		// {
		// 	s_inPixels[i + (height-1) * width] = inPixels[i + (height-1) * width];
		// }
		
		for(int r = height-2; r >= 0; r--)
		{
			// s_inPixels[i + r * width] = inPixels[i + r * width];
			// __syncthreads();		
...
__global__ void findLeastSignificantSeam(int * importancy, int * seam, int * nextPixels, int width, int height)
{
	int idx = threadIdx.x + threadIdx.y*blockDim.x + gridDim.x*blockIdx.x;
	
	if(idx < width)
	{
		if (importancy[idx] < importancy[d_min])
		{
			d_min = idx;
		}
	}
...
__global__ void deleteLeastSignificantSeamKernel(uint8_t * inPixels, int width, int height,
        	...
        	int times = 1)
{
	...
	int *d_importancy, *d_next_pixels, *d_filterX, *d_filterY, *d_seam;
	...
	CHECK(cudaMalloc(&d_importancy, width*height*sizeof(int)));
	...
	for(int count = 0; count < times; count++)
	{
		...
		edgeDetectionKernel<<<gridSize, blockSize, smem>>>(d_inGrayscale, width, height, d_filterX, d_filterY, filterWidth, d_importancy);
		cudaDeviceSynchronize();
		CHECK(cudaGetLastError());

		// CHECK(cudaMemcpy(importancy, d_importancy, width*height*sizeof(int), cudaMemcpyDeviceToHost));
		// writeTxt(importancy, width, height, "d_importancy.txt");

		//Calculate seam from pixels importancy
		calculateImportancyKernel<<<gridSize_2, blockSize>>>(d_importancy, width, height, d_next_pixels);
		cudaDeviceSynchronize();
		CHECK(cudaGetLastError());
		CHECK(cudaMemcpy(importancy, d_importancy, width*height*sizeof(int), cudaMemcpyDeviceToHost));
		CHECK(cudaMemcpy(next_pixels, d_next_pixels, width*(height-1)*sizeof(int), cudaMemcpyDeviceToHost));
		// writeTxt(importancy, width, height, "d_seam_importancy.txt");
		...
		deleteLeastSignificantSeamKernel<<<gridSize, blockSize>>>(d_inPixels, width, height, d_seam, d_outPixels);
		cudaDeviceSynchronize(); 
		CHECK(cudaGetLastError());
		width--;
