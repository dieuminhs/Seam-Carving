...
__global__ void calculateImportancyVerticalKernel(int * inPixels, int width, int height,
				int * nextPixels)
{
	// TODO
	...
	if(i < width)
	{
		...
		for(int r = height-2; r >= 0; r--)
		{
			...
			if(i-1 >= 0)
			{
				if (inPixels[i-1 + (r+1) * width] < inPixels[min])
				{
					min = i-1 + (r+1) * width;
				}
			}
...
__global__ void calculateImportancyHorizontalKernel(int * inPixels, int width, int height,
									int * nextPixels)
{
	// TODO
	// gridSize =  (Height-1)/BlockSize + 1
	int i = threadIdx.x + threadIdx.y*blockDim.x + gridDim.x*blockIdx.x;
	int min;

	if(i < height)
	{	
		// Calculate seam from right to left
		for(int c = width-2; c >= 0; c--)
		{
			// Compare the middle pixel with upper and lower pixel to find min
			min = c+1 + i * width;

			if(i-1 >= 0)
			{
				if (inPixels[c+1 + (i-1) * width] < inPixels[min])
				{
					min = c+1 + (i-1) * width;
				}
			}
			...
			// Add pixels to create the seam and save the position in nextPixels array to track
			inPixels[c + i * width] += inPixels[min];
			nextPixels[c + i * width] = min;
			__syncthreads();
		}
	}
}

__global__ void removeLeastSignificantSeamVerticalKernel(uint8_t * inPixels, int width, int height,
...
__global__ void removeLeastSignificantSeamHorizontalKernel(uint8_t * inPixels, int width, int height,
										int * seam, uint8_t * outPixels)
{
	int j = threadIdx.x + blockDim.x * blockIdx.x;
	int i = threadIdx.y + blockDim.y * blockIdx.y;
	...
	if(j < width && i < height)
	{
		if(i >= seam[j]/width)
		{
			outPixels[3*(j + i*width)] = inPixels[3*(j + (i+1)*width)];
			outPixels[3*(j + i*width) + 1] = inPixels[3*(j + (i+1)*width) + 1];
			outPixels[3*(j + i*width) + 2] = inPixels[3*(j + (i+1)*width) + 2];
		}
		else 
		{
        	...
        	int times = 1)
{
	...
	uint8_t * d_inGrayscale, *d_inPixels, *d_outPixels;
	int *importancy_v = (int*)malloc(width*height*sizeof(int));
	...
	int *d_importancy_v, *d_importancy_h, *d_next_pixels_v, *d_next_pixels_h, *d_seam_v, *d_seam_h;
	...
	CHECK(cudaMalloc(&d_importancy_v, width*height*sizeof(int)));
	CHECK(cudaMalloc(&d_importancy_h, width*height*sizeof(int)));
	CHECK(cudaMalloc(&d_next_pixels_v, width*(height-1)*sizeof(int)));
	CHECK(cudaMalloc(&d_next_pixels_h, (width-1)*height*sizeof(int)));
	CHECK(cudaMalloc(&d_seam_v, height*sizeof(int)));
	CHECK(cudaMalloc(&d_seam_h, width*sizeof(int)));

	dim3 gridSize((width-1)/blockSize.x + 1, (height-1)/blockSize.y + 1);

	// Grid size for seam calculation
	dim3 gridSize_2((width-1)/(blockSize.x*blockSize.y) + 1);
	dim3 gridSize_3((height-1)/(blockSize.x*blockSize.y) + 1);
	...
	for(int count = 0; count < times; count++)
	{
		...
		edgeDetectionKernel<<<gridSize, blockSize, smem>>>(d_inGrayscale, width, height, filterWidth, d_importancy_v);
		cudaDeviceSynchronize();
		CHECK(cudaGetLastError());

		// // Test importancy
		// CHECK(cudaMemcpy(importancy_v, d_importancy_v, width*height*sizeof(int), cudaMemcpyDeviceToHost));
		// writeTxt(importancy_v, width, height, "d_importancy.txt");

		CHECK(cudaMemcpy(d_importancy_h, d_importancy_v, width*height*sizeof(int), cudaMemcpyDeviceToDevice);)
		//Calculate seam from pixels importancy vertically
		calculateImportancyVerticalKernel<<<gridSize_2, blockSize, 0, stream[0]>>>(d_importancy_v, width, height, d_next_pixels_v);
		cudaDeviceSynchronize();
		CHECK(cudaGetLastError());

		//Calculate seam from pixels importancy horizontally
		calculateImportancyHorizontalKernel<<<gridSize_3, blockSize, 0, stream[1]>>>(d_importancy_h, width, height, d_next_pixels_h);
		cudaDeviceSynchronize();
		CHECK(cudaGetLastError());

		// Copy from kernel memory to host memory in order to find min seam using host
		CHECK(cudaMemcpyAsync(importancy_v, d_importancy_v, width*height*sizeof(int), cudaMemcpyDeviceToHost, stream[0]));
		CHECK(cudaMemcpyAsync(importancy_h, d_importancy_h, width*height*sizeof(int), cudaMemcpyDeviceToHost, stream[1]));
		CHECK(cudaMemcpyAsync(next_pixels_v, d_next_pixels_v, width*(height-1)*sizeof(int), cudaMemcpyDeviceToHost, stream[0]));
		CHECK(cudaMemcpyAsync(next_pixels_h, d_next_pixels_h, (width-1)*height*sizeof(int), cudaMemcpyDeviceToHost, stream[1]));

		// writeTxt(importancy_h, width, height, "d_seam_importancy.txt");

		//Find min seam
		int min_h = 0, min_v = 0;
		for(int c = 1; c < width; c++)
		{
			if(importancy_v[c] < importancy_v[min_v]) 
			{
				min_v = c;
			} 
		}
		...
		{
			if(importancy_h[r*width] < importancy_h[min_h])
			{
				min_h = r*width;
			}	
		}

		// Check whether to remove Vertical or Horizontal seam 
		if(importancy_h[min_h] >= importancy_v[min_v])
		{
			useVertical = true;
			min_seam[0] = min_v;
		...
		else
		{
			...
			{
				min_seam[c+1] = next_pixels_h[min_h];
				min_h = min_seam[c+1];
			}
		}
		...
		// {
		// 	temp[3*min_seam[i]] = 200;
		// 	temp[3*min_seam[i] + 1] = 200;
		// 	temp[3*min_seam[i] + 2] = 200;
		// }
		// writePnm(temp, 3, width, height, "d_seam_found.pnm");	
		
		//Remove min seam from the image
		if(useVertical)
		{
			CHECK(cudaMemcpy(d_seam_v, min_seam, height*sizeof(int), cudaMemcpyHostToDevice));
			removeLeastSignificantSeamVerticalKernel<<<gridSize, blockSize>>>(d_inPixels, width, height, d_seam_v, d_outPixels);
			width--;
		}
		else
		{
			CHECK(cudaMemcpy(d_seam_h, min_seam, width*sizeof(int), cudaMemcpyHostToDevice));
			removeLeastSignificantSeamHorizontalKernel<<<gridSize, blockSize>>>(d_inPixels, width, height, d_seam_h, d_outPixels);
			height--;
			CHECK(cudaMemcpyToSymbol(idx_h, &zero, sizeof(int)));
		}
		cudaDeviceSynchronize(); 
		CHECK(cudaGetLastError());
	...
	writePnm(outPixels, 3, width, height, "d_seam_removed.pnm");
}
	...
	int * filterX, int * filterY, int filterWidth, int times)
{
	...
	for(int count = 0; count < times; count++)
	{
		...
		for (int resultR = 0; resultR < height; resultR++)
		{
			for (int resultC = 0; resultC < width; resultC++)
			{
				...
				importancy_v[resultR*width + resultC] = abs(importancy_X) + abs(importancy_Y);
			}
		}
		// writeTxt(importancy, width, height, "importancy.txt");
		memcpy(importancy_h, importancy_v, width*height*sizeof(int));
		...
		for(int r = height - 2; r >= 0; r--)
		{
			for(int c = 0; c < width; c++)
			{
				...
				for (int idx = c-1; idx <= c+1; idx+=2)
				{
					...
					if(importancy_v[idx + (r+1)*width] < importancy_v[min])
					{
						min = idx + (r+1)*width;
					}

				}
				importancy_v[c + r*width] += importancy_v[min];			
		...
		for(int c = width - 2; c >= 0; c--)
		{
			...
			{
				min = c+1 + r*width;
				for (int idx = r-1; idx <= r+1; idx+=2)
				{
					if(idx < 0) idx = 0;
					if(idx > height-1) idx = height - 1;
					if(importancy_h[c+1 + idx*width] < importancy_h[min])
					{
						min = c+1 + idx*width;
					}

				}
				importancy_h[c + r*width] += importancy_h[min];			
				next_pixels_h[c + r*width] = min;							
			}	
		}
		...
		for(int c = 1; c < width; c++)
		{
			if(importancy_v[c] < importancy_v[min_v]) min_v = c; 
		}

		for(int r = 1; r < height; r++)
		{
			if(importancy_h[r*width] < importancy_h[min_h]) min_h = r*width;
		}
		...
		// for(int i = 0; i < height; i++)
		// {
		// 	inPixels[3*min_seam[i]] = 200;
		// 	inPixels[3*min_seam[i] + 1] = 200;
		// 	inPixels[3*min_seam[i] + 2] = 200;
		// }
		// writePnm(inPixels, 3, width, height, "seam_found.pnm");
	...
	writePnm(outPixels, 3, width, height, "seam_removed.pnm");
}
