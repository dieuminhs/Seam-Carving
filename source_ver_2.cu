	...
	if(j < width && i < height)
	{
		...
		for (int r = 0; r < filterWidth; r++)
		{
			for (int c = 0; c < filterWidth; c++)
			{
				...
				result[0] += org_pixel * filterX[c + r * filterWidth];
				result[1] += org_pixel * filterY[c + r * filterWidth];
			}
		}
...
__global__ void deleteLeastSignificantSeamVerticalKernel(uint8_t * inPixels, int width, int height,
...
__global__ void deleteLeastSignificantSeamHorizontalKernel(uint8_t * inPixels, int width, int height,
										int * seam, uint8_t * outPixels)
{
	int j = threadIdx.x + blockDim.x * blockIdx.x;
	int i = threadIdx.y + blockDim.y * blockIdx.y;
        	...
        	int times = 1)
{
	...
	int * next_pixels_v = (int*)malloc(width*height*sizeof(int));
	int * next_pixels_h = (int*)malloc(width*height*sizeof(int));
	int *d_importancy_v, *d_importancy_h, *d_next_pixels_v, *d_next_pixels_h, *d_filterX, *d_filterY, *d_seam_v, *d_seam_h;
	...
	CHECK(cudaMalloc(&d_next_pixels_v, width*height*sizeof(int)));
	CHECK(cudaMalloc(&d_next_pixels_h, width*height*sizeof(int)));
	...
	for(int count = 0; count < times; count++)
	{
		...
		edgeDetectionKernel<<<gridSize, blockSize, smem>>>(d_inGrayscale, width, height, d_filterX, d_filterY, filterWidth, d_importancy_v);
		cudaDeviceSynchronize();
		CHECK(cudaGetLastError());
		...
		calculateImportancyVerticalKernel<<<gridSize_2, blockSize>>>(d_importancy_v, width, height, d_next_pixels_v);
		cudaDeviceSynchronize();
		CHECK(cudaGetLastError());

		//Calculate seam from pixels importancy horizontally
		calculateImportancyHorizontalKernel<<<gridSize_3, blockSize>>>(d_importancy_h, width, height, d_next_pixels_h);
		cudaDeviceSynchronize();
		CHECK(cudaGetLastError());

		CHECK(cudaMemcpy(importancy_v, d_importancy_v, width*height*sizeof(int), cudaMemcpyDeviceToHost));
		CHECK(cudaMemcpy(importancy_h, d_importancy_h, width*height*sizeof(int), cudaMemcpyDeviceToHost));
		CHECK(cudaMemcpy(next_pixels_v, d_next_pixels_v, width*height*sizeof(int), cudaMemcpyDeviceToHost));
		CHECK(cudaMemcpy(next_pixels_h, d_next_pixels_h, width*height*sizeof(int), cudaMemcpyDeviceToHost));
		...
		if(useVertical)
		{
			...
			deleteLeastSignificantSeamVerticalKernel<<<gridSize, blockSize>>>(d_inPixels, width, height, d_seam_v, d_outPixels);
			cudaDeviceSynchronize(); 
			CHECK(cudaGetLastError());
			width--;
		}
		else
		{
			...
			deleteLeastSignificantSeamHorizontalKernel<<<gridSize, blockSize>>>(d_inPixels, width, height, d_seam_h, d_outPixels);
			cudaDeviceSynchronize(); 
			CHECK(cudaGetLastError());
			height--;
	...
	int * filterX, int * filterY, int filterWidth, int times)
{
	...
	int * importancy = (int*)malloc(width*height*sizeof(int));
	...
	for(int count = 0; count < times; count++)
	{
		...
		for (int resultR = 0; resultR < height; resultR++)
		{
			for (int resultC = 0; resultC < width; resultC++)
			{
				...
				importancy[resultR*width + resultC] = abs(importancy_X) + abs(importancy_Y);
			}
		}
		...
		for(int r = height - 2; r >= 0; r--)
		{
			for(int c = 0; c < width; c++)
			{
				...
				for (int idx = c-1; idx <= c+1; idx+=2)
				{
					...
					if(importancy[idx + (r+1)*width] < importancy[min])
					{
						min = idx + (r+1)*width;
					}

				}
				importancy[c + r*width] += importancy[min];			
		...
		for(int c = 1; c < width; c++)
		{
			if(importancy[c] < importancy[min]) min = c; 
		}
		min_seam[0] = min;
	...
	outPixels = inPixels;
	writePnm(inPixels, 3, width, height, "seam_removed.pnm");
}
